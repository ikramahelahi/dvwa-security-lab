...
### LOW
The session ID is generated by starting at 0 and increasing the number by 1 for each new session.

### MEDIUM
The session ID is based on the Unix timestamp, which represents the exact time when the session was created.

### HIGH
The session ID starts as a number that increases from 0, and then it is converted into an MD5 hash to make it harder to guess.
